...
COLUMNS = ["Seq", "Type", "ThDist", "WDist", "HuVGS", "WVGS",
           # "ThBerta",
           "WBerta",
           # "ThAbody",
           "WAbody",
           ...
           # "WSap",
           # "ThAblang",
           "WAblang", "OASId", "OASPerc", "HumCM", "MurCM", "RheCM"]
...
def print_info(seq: str, way: str, v_gene_scorer, wild_v_gene_scorer, biophi_path, models, wild_models,
    ...
    a_seq = annotate_single(seq, ChothiaHeavy(), GeneralChainType.HEAVY)
    ...
    args = [
        ...
        round(models[v_gene_type(v_gene_scorer, a_seq)].predict_proba([a_seq])[0, 1], 2) if a_seq is not None else "",
    ]
    args.extend([round(model.predict_proba([a_seq])[0, 1], 2) for model in wild_models if a_seq is not None])
...
def main(files, dataset, wild_dataset, biophi_path, human_models_dir, murine_models_dir, rhesus_models_dir):
    ...
    for mab, lst in seqs.items():
        ...
        sap_emb_thera, sap_emb_wild = None, None
